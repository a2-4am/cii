...
--[[
  This tests copying a file to a DOS 3.3-formatted disk where the directory is full (already contains the maximum number of items). The operation should fail with an error message, matching v8.4 behavior.
]]

--[[ BEGINCONFIG ========================================

  MODELARGS="-sl5 'diskiing'"
  DISKARGS="-flop1 'directory-full-dos33.do' -flop2 'directory-full-dos33.do' -flop3 $FLOPIMG -flop4 'DOS 3.3 System Master.do'"
...
test.Step(
  "Copy file to full directory (DOS 3.3) fails, matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
    ...
    apple2.TypeLine("HELLO") -- will match 1 file
    cii.WaitForScreenContains("%[RETURN]%-MARK FILE, %[U]NMARK, %[E]NTER")
    apple2.Type("G") -- Go
    ...
    test.ExpectBinaryEquals(util.SlurpFile(target_filename),
                            ...
                            "Failed copy file to full DOS 3.3 directory unexpectedly modified target disk")
end)
