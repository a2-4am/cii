...
--[[
  This tests reformatting a 5.25-inch disk in DOS 3.3 format that has already been formatted as ProDOS. The operation should succeed (after confirming destruction of the old disk by volume name). The operation should succeed, however the final disk image may not be identical to one where the same operation was performed by v8.4, because v8.4 had an off-by-1 bug in its no-boot memory copy loop that ended up storing garbage in byte $A2 of T00S00 of DOS 3.3-formatted disks. C2Reboot fixes this bug, hence the disk images may differ on that one specific byte. Any other differences are a regression.and write ProDOS boot blocks and a blank ProDOS root directory with the new volume name no files. The final disk image should be identical to one where the same operation was performed by v8.4.
]]
...
test.Step(
  "Format Disk (DOS 3.3) of ProDOS disk matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
