...
--[[
  This tests renaming a directory on a ProDOS disk. The operation should succeed, matching v8.4 behavior. The final disk image should be identical to one where the same operation was performed by v8.4.
]]
...
--[[ BEGINCONFIG ========================================
  ...
  DISKARGS="-flop1 'beagle-graphics-renamed-directory-by-v84.do' -flop3 $FLOPIMG -flop4 'beagle-graphics.do'"
...
test.Step(
  "Rename directory (ProDOS) matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
    ...
    apple2.TypeLine("SUBDIRECTORY") -- will match SUBDIRECTORY
    cii.WaitForScreenContains("%[RETURN]%-SELECT TO RENAME")
    apple2.ReturnKey()
    cii.WaitForScreenContains("RENAME SUBDIRECTORY")
    apple2.TypeLine("STUFF")
    cii.WaitForScreenContains("%[RETURN]%-SELECT TO RENAME")
    apple2.Type("G") -- Go
    cii.WaitForMainMenu()
    ...
    test.ExpectBinaryEquals(util.SlurpFile(source_filename),
                            ...
                            "Rename directory (ProDOS) disk image differs from v8.4")
end)
