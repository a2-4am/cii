...
--[[
  This tests View Files as Values on a binary file from a DOS 3.3 formatted disk, matching v8.4 behavior.
]]
...
test.Step(
  "View Files as Values on binary file (DOS 3.3) matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
    ...
    apple2.TypeLine("HELLO PRONTO-DOS") -- will match that file
    cii.WaitForScreenContains("%[E]NTER FILENAME, %[G]O, %[ESC]%-EXIT")
    ...
    test.ExpectIMatch(apple2.GrabTextScreen(), "00 40 58 04 08 78 A2 05  %.@X%.%.X\"%.", "View Files as Values behavior does not match v8.4")
    test.ExpectIMatch(apple2.GrabTextScreen(), "42 85 03 A2 00 20 9E 40  B%.%.\"%. %.@", "View Files as Values behavior does not match v8.4")
