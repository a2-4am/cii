...
--[[ BEGINCONFIG ========================================
  ...
  MODELARGS="-aux 'ext80' -sl1 'ramfactor'"

  ======================================== ENDCONFIG ]]

test.Step(
  "Device list on 8MB //e with RAMFactor RAM disk in slot 1 appears as Slot 1 Drive 1, matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
    ...
    test.Expect(cii.ScreenContains("SLOT 1  DRIVE 1"), "Device list does not include expected device s1d1")
    test.Expect(not cii.ScreenContains("SLOT 0"),          "Device list includes unexpected device in slot 0")
    test.Expect(not cii.ScreenContains("SLOT 1  DRIVE 2"), "Device list includes unexpected device s1d2")
