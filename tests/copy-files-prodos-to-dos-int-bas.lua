...
--[[
  This tests copying several files from a ProDOS disk to a DOS 3.3 disk, including BAS and INT files. The operation should succeed, matching v8.4 behavior. C2Reboot matches v8.4 with the exact order of files in the disk catalog, the exact placement of sectors for each file on disk, and the contents of each sector.
]]
...
--[[ BEGINCONFIG ========================================
  ...
  DISKARGS="-flop1 'formatted-dos33-by-v84.do' -flop2 'beagle-compiler-filecopied-to-DOS-by-v84.do' -flop3 $FLOPIMG -flop4 'beagle-compiler.do'"
...
test.Step(
  "Copy INT and BAS files from ProDOS disk to DOS 3.3 disk matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
    ...
    apple2.TypeLine("STARTUP=") -- will match two files, STARTUP (INT) and STARTUP.BASIC (BAS)
    cii.WaitForScreenContains("%[RETURN]%-MARK FILE, %[U]NMARK, %[E]NTER")
    apple2.Type("G") -- Go
    cii.WaitForMainMenu()
    s6d1:unload()
    test.ExpectBinaryEquals(util.SlurpFile(target_filename),
                            ...
                            "Copy INT and BAS files from ProDOS disk to DOS 3.3 disk does not match v8.4")
end)
