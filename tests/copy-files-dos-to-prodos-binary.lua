...
--[[ BEGINCONFIG ========================================
  ...
  DISKARGS="-flop1 'formatted-prodos-by-v84.do' -flop2 'DOS-binary-filecopied-to-ProDOS-by-v84.do' -flop3 $FLOPIMG -flop4 'Pronto-DOS Master.do'"
...
test.Step(
  "Copy binary files from DOS 3.3 disk to ProDOS disk matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
    ...
    apple2.TypeLine("=LOGO,B") -- will match several binary files
    cii.WaitForScreenContains("%[RETURN]%-MARK FILE, %[U]NMARK, %[E]NTER")
    apple2.Type("G") -- Go
    cii.WaitForMainMenu()
    s6d1:unload()
    test.ExpectBinaryEquals(util.SlurpFile(target_filename),
                            ...
                            "Copy binary files from DOS 3.3 disk to ProDOS disk does not match v8.4")
end)
