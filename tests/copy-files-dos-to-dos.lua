...
--[[
  This tests copying several files from one DOS 3.3 disk to another. The operation should succeed, matching v8.4 behavior. C2Reboot matches v8.4 with the exact order of files in the disk catalog, the exact placement of sectors for each file on disk, and the contents of each sector.
]]
...
test.Step(
  "Copy Files from DOS 3.3 disk to DOS 3.3 disk matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
    ...
    test.ExpectBinaryEquals(util.SlurpFile(target_filename),
                            ...
                            "Copy Files from DOS 3.3 disk to DOS 3.3 disk does not match v8.4")
end)
