...
--[[
  This tests that C2Reboot displays an error message when attempting to copy files to an unformatted disk, matching v8.4 behavior.
]]
...
test.Step(
  "Copy files to unformatted disk fails, matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
    apple2.Type("X") -- Copy
    ...
    apple2.TypeLine("=,.SYS") -- will match all SYSTEM files
    cii.WaitForScreenContains("%[RETURN]%-MARK FILE, %[U]NMARK, %[E]NTER")
    apple2.Type("G") -- Go
