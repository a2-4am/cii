...
--[[
  This tests copying a file to a disk where the file already exists. The operation should be interrupted by a prompt saying the file already exists and asking what to do. Selecting "New Name" should prompt for a new filename then continue the copy operation, copying the file to a new file with the given name, the original file's lock status, the original file's modification date, and the current date as creation date (or NO DATE if no clock). The final disk image should be identical to one where the same operation was performed by v8.4.
]]
...
--[[ BEGINCONFIG ========================================
  ...
  DISKARGS="-flop1 'beagle-graphics-filecopied-new-name-by-v84.do' -flop3 $FLOPIMG -flop4 'beagle-graphics.do'"
...
test.Step(
  "Copy Files when file exists prompts for action, selecting \"New Name\" matches v8.4 behavior",
  function()
    cii.WaitForMainMenu()
    apple2.Type("X") -- Copy
    ...
    apple2.TypeLine("STARTUP") -- will match STARTUP file only
    cii.WaitForScreenContains("%[RETURN]%-MARK FILE, %[U]NMARK, %[E]NTER")
    apple2.Type("G") -- Go
    ...
    test.ExpectBinaryEquals(
      ...
      "Copy Files with file exists, user said New Name, but disk did not match v8.4 behavior")
end)
